...
class SmpsAdvSettings(models.TransientModel):
    _name = "smps.adv.settings"
    _description = "SO Multi Product Selection Advanced Settings"
    ...
    def get_sh_smps_pro_field_ids(self):
        if(
            ...
            self.env.user.company_id.sh_smps_pro_field_ids
        ):
    ...
    def get_sh_smps_pro_attr_ids(self):
        if(
            self.env.user.company_id and
            self.env.user.company_id.sh_smps_pro_attr_ids
        ):
    ...
    sh_smps_pro_field_ids = fields.Many2many(
        ...
        domain=[
            ('model_id.model', 'in', ['product.product', 'product.template']),
