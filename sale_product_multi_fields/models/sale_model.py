...
class SaleModel(models.Model):
    ...
    def sh_smps_adv_btn(self):
        if self:
            view = self.env.ref(
                'sale_product_multi_fields.sh_smps_adv_wizard_form_view'
            )
            ori_arch = """
...
<form string="Select Products Advance">
                    ...
                    for rec in sh_cid.sh_smps_pro_field_ids:
                        ...
                        if search_field and search_field.ttype == 'selection':
                            ...
                            selection_key_value_list = self.env[
                                rec.model
                            ].sudo()._fields[rec.name].selection
                ...
                if sh_cid.sh_smps_pro_attr_ids:
                    for rec in sh_cid.sh_smps_pro_attr_ids:
                        ...
                        middle_str += "<div class='row' style='border-bottom:1px solid #ccc; margin:5px 10px;margin: 9px 0px;'>"
                        middle_str += "<div class='col-sm-3 text-right' style='font-weight:bold'> <label for='x_attr_" + str(
                            rec.id) + "'/></div>"
                        middle_str += "<div class='col-sm-3'> <field name='x_attr_" + str(
                            rec.id) + "' " + domain_str + ' ' + no_create_str + "/></div>"
...
class SaleOrderLine(models.Model):
    ...
    mo_company_no = fields.Char(string="رقم التصنيع او رقم الشركه المصنعه", store=True, related="product_id.mo_company_no")
    # -----------------------------------------------------------------------
    ...
    product_description_models = fields.Char(string='وصف الصنف', store=True, related="product_id.product_description_models")
